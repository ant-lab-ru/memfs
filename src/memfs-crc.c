...
#define MEMFS_MPEG2_CRC32_INIT_VALUE (0xffffffff)

uint32_t mpeg2_crc32 (uint8_t *data, uint32_t size, uint32_t start_value)
{
    uint32_t crc = start_value;
    for (uint32_t i = 0; i < size; i++) {
        crc = (crc << 8) ^ mpeg2_crc_table[((crc >> 24) ^ *data++) & 0xff];
    }
...
{
    uint32_t offset = 0;
    uint32_t crc = MEMFS_MPEG2_CRC32_INIT_VALUE;
    ...
    case MEMFS_ATTR_REGULAR:
        ...
        while (1)
        {
            ...
            uint32_t read_size = MEMFS_MIN(MEMFS_DATA_BUF_LEN, f->size - read_addr);
            ...
            crc = mpeg2_crc32(memfs_ctx.data_buf, rc, crc);
            if (read_size < MEMFS_DATA_BUF_LEN) {
                return crc;
            }
        }
        break;
